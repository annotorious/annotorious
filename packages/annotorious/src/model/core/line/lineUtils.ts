...
const LineUtil: ShapeUtil<Line> = {
  ...
  intersects: (l: Line, x: number, y: number, scale: number = 1): boolean => {
    const [[x1, y1], [x2, y2]] = l.geometry.points;

    // Maximum distance in pixels (adjusted with scale) at which we consider the point intersects with the line
