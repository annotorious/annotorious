<script lang="ts">
  ...
  import SVGPresencePoint from './shapes/PresencePoint.svelte';
  ...
  const onSelectionChange = (p: PresentUser, selection: string[] | null) => {
    trackedAnnotations = [
      ...trackedAnnotations.filter(({ selectedBy }) => selectedBy.presenceKey !== p.presenceKey),
      ...(selection || []).map((id) => ({
      ...
      if (!annotation) console.warn('Selection event on unknown annotation');
...
{#if Boolean(provider)}
  <OSDLayer {viewer} let:transform let:scale>
    <svg class="a9s-osd-presencelayer">
      <g {transform}>
        {#if trackedAnnotations.length > 0}
          {#each trackedAnnotations as tracked}
            {#if tracked.annotation.target.selector.type === ShapeType.RECTANGLE}
              <SVGPresenceRectangle annotation={tracked.annotation} user={tracked.selectedBy} {scale} />
            {:else if tracked.annotation.target.selector.type === ShapeType.POLYGON}
              <SVGPresencePolygon annotation={tracked.annotation} user={tracked.selectedBy} {scale} />
            {:else if tracked.annotation.target.selector.type === ShapeType.POINT}
              <SVGPresencePoint annotation={tracked.annotation} user={tracked.selectedBy} {scale} />
            {/if}
          {/each}
        {/if}
      </g>
